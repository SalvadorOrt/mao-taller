...
def usuario_puede_acceder_inventario(
    request,
    inventario,
):
    """
    Valida el alcance de sucursal del inventario físico.

    Puede acceder cuando:
    - tiene permiso global para cambiar/consultar sucursal, o
    - el inventario pertenece a su sucursal activa/asignada.
    """
...
def negar_acceso_inventario(request):
    messages.error(
        request,
        "No tienes permisos para acceder a ese inventario físico.",
    )
...
def inventario_fisico_lista(request):
        ...
        if sucursal_id.isdigit():
            ...
            if sucursal_solicitada:
                inventarios = inventarios.filter(
                    sucursal=sucursal_solicitada
                )
            else:
                sucursal_id = ""

        elif sucursal_id:
            # Evita aceptar valores manipulados por URL.
            sucursal_id = ""
    ...
    if q:
        inventarios = inventarios.filter(
            ...
            |
            Q(
                sucursal__nombre__icontains=q
            )
            |
            Q(
                sucursal__codigo__icontains=q
            )
        )

    return render(
        request,
        "inventario/fisico/lista.html",
        {
            ...
            "estados": (
                InventarioFisico.ESTADO_CHOICES
            ),
        },
    )
...
@permiso_requerido(
    "inventario.add_inventariofisico"
)
    ...
    if request.method == "POST":
        ...
        if puede_cambiar_sucursal:
            ...
            if not sucursal_id.isdigit():
                ...
                return render(
                    request,
                    "inventario/fisico/form_crear.html",
                    {
                        "sucursales": sucursales,
                        "sucursal_activa": sucursal_activa,
                        "puede_cambiar_sucursal": (
            ...
            if not sucursal:
                messages.error(
                    request,
                    (
                        "Tu usuario no tiene una sucursal "
                        "asignada para crear el inventario."
                    ),
                )
...
def inventario_fisico_detalle(
    ...
    if not usuario_puede_acceder_inventario(
        request,
        inventario,
    ):
        return negar_acceso_inventario(
            request
        )
    ...
    if marca_id:
        detalles = detalles.filter(
            codigo_producto__marca_id=marca_id
        )
    ...
    return render(
        request,
        "inventario/fisico/detalle.html",
        {
...
@permiso_requerido(
    "inventario.change_inventariofisico"
)
def inventario_fisico_generar_detalles(
    request,
    inventario_id,
):
    inventario = get_object_or_404(
    ...
    if inventario.estado not in [
        ...
        messages.error(
            request,
            (
                "Solo se pueden generar detalles "
                "en inventarios abiertos."
            ),
        )
    ...
    if not inventario.sucursal_id:
        messages.error(
            request,
            (
                "El inventario físico debe tener "
                "una sucursal."
            ),
        )
    ...
    with transaction.atomic():
        for stock in stocks:
            _, creado = (
                DetalleInventarioFisico.objects
                .get_or_create(
                    inventario=inventario,
                    codigo_producto=(
...
def inventario_fisico_agregar_linea(
    request,
    inventario_id,
):
    inventario = get_object_or_404(
    ...
    if request.method == "POST":
        ...
        codigo = (
            ...
            .filter(
                Q(
                    codigo__iexact=codigo_txt
                )
                |
                Q(
                    codigo_normalizado__iexact=(
                ...
                |
                Q(
                    codigo_barras__iexact=codigo_txt
                )
            )
            .first()
        )
        ...
        if not codigo:
            messages.error(
                ...
                (
                    ...
                    "con ese código o código de barras."
                ),
            )
        ...
        try:
            ...
            messages.success(
                request,
                "Línea de inventario actualizada.",
            )
...
def inventario_fisico_actualizar_conteo(
    request,
    detalle_id,
):
    ...
    if request.method == "POST":
        ...
        try:
            detalle.cantidad_contada = Decimal(
                cantidad_contada
                or "0"
            )
...
def inventario_fisico_cerrar(
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            (
                "Inventario físico cerrado. "
                "Revise diferencias antes de "
                "aplicar ajustes."
            ),
        )
...
def inventario_fisico_aplicar_ajustes(
    request,
    inventario_id,
):
    inventario = get_object_or_404(
    ...
    # Aplicar el inventario genera movimientos de stock.
    # Por eso también exigimos permiso para crear movimientos.
    if not (
        ...
        messages.error(
            request,
            (
                "No tienes permisos para aplicar "
                "ajustes al stock."
            ),
        )
    ...
    if inventario.estado != "cerrado":
        messages.error(
            request,
            (
                "Solo se pueden aplicar ajustes "
                "desde un inventario cerrado."
            ),
        )
